...
from Jobs_pb2 import _JOBSTATUS
from Jobs_pb2 import JobListRequest
from Jobs_pb2 import JobListReply
from Jobs_pb2 import JobDescriptor
from Jobs_pb2 import JobAccepted
from Jobs_pb2 import JobRejected
from Workflows_pb2 import WorkflowListRequest
from Workflows_pb2 import WorkflowListReply
from Service_pb2 import Parameter
...
class CommandDispatcher(object):
				...
				for p in par:
					...
					if (m is None or len(m.groups()) != 3 or len(m.group(1)) == 0 or len(m.group(2)) == 0 or len(m.group(3)) == 0):
						if self.verbose:
							...
							raise CommandDispatcherMessage("incorrect parameter '%s'. Expected pattern is 'service_label.param_name=param_value'" % p)
						sys.exit()
					label = m.group(1)
					par = Parameter()
					par.name = m.group(2)
	...
	def _command_config_get(self):
		...
		from Config_pb2 import GetConfigReply
		if self.verbose:
	...
	def _command_config_set(self):
		...
		from Config_pb2 import SetConfigReply
		from Config_pb2 import Property
	...
	def _command_job_details(self):
		...
		'''
		from Info_pb2 import InfoRequest
		from Info_pb2 import InfoData
		from Info_pb2 import InfoError
		if self.verbose:
			print "Job details called"
		request = InfoRequest()
		from Info_pb2 import JOB
		request.type = JOB
		...
		if mtype == "InfoError":
			...
			for att in infoData.data.attrs:
				if (att.type == 2):
					...
					attributes[att.name] = datetime.datetime.fromtimestamp(ts).strftime("%Y-%m-%d %H:%M:%S")
					datetimeAttributes[att.name] = datetime.datetime.fromtimestamp(ts)
				elif (att.type == 5):
	...
	def _command_workflow_list(self):
		...
		if mtype == "WorkflowListReply":
			...
			from Workflows_pb2 import WorkflowError
			werr = WorkflowError()
	...
	def _command_workflow_upload(self):
		...
		'''
		from Workflows_pb2 import WorkflowUploadRequest
		from Workflows_pb2 import WorkflowUploadReply
		aw = WorkflowUploadRequest()
	...
	def _command_workflow_get(self):
		...
		'''
		from Workflows_pb2 import WorkflowGetRequest
		from Workflows_pb2 import WorkflowGetReply
		gw = WorkflowGetRequest()
		...
		if  mtype == "WorkflowGetReply":
			...
			try:
				...
				self.logger.warn("Couldn't open destination file '%s'. Printing to stdout:" % e.message)
				sys.stdout.write(gwr.content)
		else:
			msg = "Unexpected response received (type=%s)." % mtype
			if self.verbose:
	...
	def _command_workflow_policy(self, enabled):
		...
		'''
		from Workflows_pb2 import WorkflowPolicyRequest
		from Workflows_pb2 import WorkflowPolicyReply
		pw = WorkflowPolicyRequest()
	...
	def _command_workflow_history(self):
		...
		'''
		from Workflows_pb2 import WorkflowHistoryRequest
		from Workflows_pb2 import WorkflowHistoryReply
		hw = WorkflowHistoryRequest()
		...
		if mtype == "WorkflowHistoryReply":
			hwr = WorkflowHistoryReply()
			hwr.ParseFromString(resp)
			if self.verbose:
				head = "Found %s revisions of '%s':" % (len(hwr.history), hw.name)
				print
				print head
				print "-" * len(head)
	...
	def _command_workflow_status(self):
		...
		'''
		from Workflows_pb2 import WorkflowStatusRequest
		from Workflows_pb2 import WorkflowStatusReply
		sw = WorkflowStatusRequest()
		...
		if mtype == "WorkflowStatusReply":
			swr = WorkflowStatusReply()
			swr.ParseFromString(resp)
			if swr.valid: valid = "valid"
			else: valid = "invalid"
			if swr.enabled: enabled = "enabled"
	...
	def _getJobStatusString(self, status):
		...
		'''
		return _JOBSTATUS.values_by_number[status].name
