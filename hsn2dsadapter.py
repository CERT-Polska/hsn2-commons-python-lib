...
class HSN2DataStoreAdapter():
	...
	def putBytes(self, bytes, job_id):
		'''
		...
		try:
			...
			http.putheader("Content-Length", str(len(bytes)))
			http.endheaders()
			...
			if errcode != 201:
				raise DataStoreException("%d - %s - %s" % (errcode, errmsg, headers))
		...
		return m.group(0); '''return new event id'''
	...
	def getFile(self, job_id, event_id):
		...
		http = httplib.HTTP(self.address)
		try:
			http.putrequest("GET", "/data/" + str(job_id) + "/" + str(event_id))
			http.putheader("User-Agent", "python service")
			http.endheaders()
			...
			if errcode != 200:
				raise DataStoreException("U" + "%d - %s - %s" % (errcode, errmsg, headers))
		except Exception as e:
			raise DataStoreException(str(e))
	...
	def saveTmp(self, job_id, event_id, prefix = 'tmp', directory = None):
		...
		(fhandle, fpath) = tempfile.mkstemp(".tmp", prefix = prefix, dir = directory, text = False)
