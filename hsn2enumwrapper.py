...
def loadList(obj, listName):
	...
	if NameToNumber.get(listName) is None:
		NameToNumber[listName] = dict([(item.name, item.number) for item in obj.DESCRIPTOR.enum_types_by_name[listName].values])
	if NumberToName.get(listName) is None:
		NumberToName[listName] = dict(zip(NameToNumber[listName].values(), NameToNumber[listName].keys()))
