...
class HSN2Service(object):
	...
	def importMapping(self, modulePath):
		...
		if moduleExt != ".py":
			logging.error("Module isn't a python file. Extension is '%s'" % moduleExt)
			return None
...
def startService(serviceType = HSN2Service, taskProcessor = None):
	...
	import loggingSetup
	loggingSetup.setupLogging(logPath = "/var/log/hsn2/%s.log" % cliargs.serviceName, logToStream = True, logLevel = cliargs.logLevel)
...
if __name__ == '__main__':
	from hsn2taskprocessor import HSN2TaskProcessor as Processor
